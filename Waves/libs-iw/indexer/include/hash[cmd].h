...
#define MAX_COLUNAS 10  // Numero maximo de letra em uma palavra que iremos adotar obs:(pode ser menor que qntd de letras de uma palavra)
 ...
 int tabHashAleatorio[256][MAX_COLUNAS]; //Tabela de cada letra em cada posição da palavra
...
void Inserir_Hash(TipoNo **Hash,  char *palavra)
{
 ...
 int Pos_Hash( char *palavra, int tamanho_hash) {
...
void parse_Hash(FILE *arquivo, FILE *arq) {
      ...
      char texto[BLOCO_TAMANHO],palavra[NUMERO];
