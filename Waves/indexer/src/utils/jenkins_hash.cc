...
inline void __jenkins_hash_vector(unsigned seed, const char *k, unsigned keylen, unsigned * hashes)
{
	...
	while (len >= 12)
	{
		hashes[0] += (k[0] +((unsigned)k[1]<<8) +((unsigned)k[2]<<16) +((unsigned)k[3]<<24));
		hashes[1] += (k[4] +((unsigned)k[5]<<8) +((unsigned)k[6]<<16) +((unsigned)k[7]<<24));
		hashes[2] += (k[8] +((unsigned)k[9]<<8) +((unsigned)k[10]<<16)+((unsigned)k[11]<<24));
...
void jenkins_state_dump(jenkins_state_t *state, char **buf, unsigned *buflen)
{
	...
	*buf = (char *)malloc(sizeof(unsigned));
	if (!*buf) 
	{
		*buflen = UINT_MAX;
		return;
	}
	memcpy(*buf, &(state->seed), sizeof(unsigned));
	return;
}
...
void jenkins_state_pack(jenkins_state_t *state, void *jenkins_packed)
{
	...
	{
		memcpy(jenkins_packed, &(state->seed), sizeof(unsigned));
	}
}
...
{
	unsigned hashes[3];
	__jenkins_hash_vector(*((unsigned *)jenkins_packed), k, keylen, hashes);
	return hashes[2];
}
